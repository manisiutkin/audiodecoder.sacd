...
public:
	...
	void init(real_t* p_fir_coefs, size_t p_fir_length, size_t p_decimation) {
		...
		int specSize{ 0 };
		int bufSize{ 0 };
		fir_status = ippsFIRMRGetSize(fir_length, 1, decimation, is_fp32 ? ipp32f : ipp64f, &specSize, &bufSize);
		fir_buf = ippsMalloc_8u(bufSize);
		fir_spec = reinterpret_cast<IppsFIRSpec*>(ippsMalloc_8u(specSize));
		if constexpr(is_fp32) {
			fir_status = ippsFIRMRInit_32f(fir_coefs, fir_length, 1, 0, decimation, 0, fir_spec);
		}
		else {
			fir_status = ippsFIRMRInit_64f(fir_coefs, fir_length, 1, 0, decimation, 0, fir_spec);
		}
 	}
	void free() {
		if (fir_dly) {
