...
public:
  ...
  bool load_info(const std::vector<uint8_t>& buffer, kodi::addon::AudioDecoderInfoTag& info);
  bool save_info(std::vector<uint8_t>& buffer, const kodi::addon::AudioDecoderInfoTag& info);
  ...
  bool set_info(size_t track_number, const kodi::addon::AudioDecoderInfoTag& info);
  void update_tags();
  bool load_info(size_t track_index, kodi::addon::AudioDecoderInfoTag& info);
  bool save_info(size_t track_index, const kodi::addon::AudioDecoderInfoTag& info);
  void update_tags(size_t track_index);
  void get_albumart(size_t albumart_id, std::vector<uint8_t>& albumart_data);
  void set_albumart(size_t albumart_id, const std::vector<uint8_t>& albumart_data);
};
