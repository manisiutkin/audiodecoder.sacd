...
bool id3_tagger_t::load_info(const std::vector<uint8_t>& buffer, kodi::addon::AudioDecoderInfoTag& info)
{
  if (buffer.empty())
    return false;

  ID3v2_tag* tag = load_tag_with_buffer((char*)buffer.data(), (int)buffer.size());
  if (!tag)
    return false;
...
bool id3_tagger_t::save_info(std::vector<uint8_t>& buffer, const kodi::addon::AudioDecoderInfoTag& info)
{
  return false;
}
